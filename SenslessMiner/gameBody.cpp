...
void GameBody::start(unsigned int x, unsigned int y)
{
	...
	while (true)
	{
		...
		if (choice == 87 || choice == 119) {
			controller.playerUP(gameField.field, playerPOS, x);
		}
		//S
		if (choice == 83 || choice == 115) {
			controller.playerDOWN(gameField.field, playerPOS, x);
		}
		//A
		if (choice == 65 || choice == 97) {
			controller.playerLEFT(gameField.field, playerPOS, x);
		}
		//D
		if (choice == 68 || choice == 100) {
			controller.playerRIGHT(gameField.field, playerPOS, x);
		}
		interface.showField(gameField.field, x);
	}
